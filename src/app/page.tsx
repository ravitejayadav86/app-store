  ...
  return (
    <div className="bg-surface text-on-surface min-h-screen font-sans -mt-24 pt-[88px] md:pt-[100px] pb-safe overflow-x-hidden relative z-10 w-full feel-play-store">
      <motion.main 
        ...
        initial="hidden"
        animate="visible"
        className="px-5 py-6 pb-32 max-w-7xl mx-auto space-y-12"
      >
                ...
                <div className="absolute bottom-0 inset-x-0 p-6 md:p-10 bg-gradient-to-t from-[#0b1224] via-[#0b1224]/80 to-transparent pt-32 flex justify-between items-end">
                   ...
                   <div className="flex flex-col items-end gap-1.5">
                       <Button
                         ...
                         className={!featuredApp.file_path ? "bg-gray-500/50 text-white/50 cursor-not-allowed" : "bg-[#8AB4F8] text-[#131314]"}
                       >
        ...
        <motion.section variants={itemVariants} className="space-y-6 max-w-4xl">
           ...
           {filteredApps.length > 0 ? (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                {filteredApps.map((app, i) => (
                   <Link key={app.id} href={`/apps/${app.id}`} className="flex items-center gap-5 cursor-pointer hover:bg-surface-low p-3 -mx-3 rounded-2xl transition-colors group app-card">
                      <div className={`w-[84px] h-[84px] rounded-[22px] ${getCategoryColor(app.category)} flex items-center justify-center shadow-sm border border-outline-variant/30 flex-shrink-0 text-3xl font-bold text-white`}>
