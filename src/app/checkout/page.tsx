...
import { CreditCard, ShieldCheck, Lock, CheckCircle2, ArrowLeft, Globe, ArrowRight, Loader2 } from "lucide-react";
...
function CheckoutContent() {
  ...
  if (!item) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-surface px-4 text-center">
      ...
      <p className="text-on-surface-variant mb-8 max-w-md">Please select an application from the store to proceed.</p>
      <Link href="/"><Button size="lg">Explore Store</Button></Link>
    </div>
  );
    ...
    <div className="flex flex-col gap-12 pb-20">
      <section className="px-4 md:px-8">
        <div className="relative h-[400px] w-full max-w-7xl mx-auto rounded-3xl overflow-hidden bg-linear-to-br from-surface-low to-surface p-12 text-on-surface flex flex-col justify-end gap-2 shadow-inner">
          ...
          <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-5xl md:text-7xl font-bold tracking-tight text-on-surface">
            Review & Finalize.
          </motion.h1>
          <p className="text-lg md:text-xl text-on-surface-variant max-w-xl">Complete your curation by securing this premium addition to your library.</p>
        </div>
      </section>
          ...
          <section className="space-y-6">
            ...
            <div className="bg-surface-low/30 p-8 rounded-3xl border border-outline-variant/50 relative overflow-hidden">
              ...
              <p className="text-on-surface text-lg font-medium leading-relaxed max-w-md">
                123 Digital Avenue, Phase 4<br />Silicon Valley, CA 94025<br />United States Premium Region
              </p>
          ...
          <div className="sticky top-24 p-1 rounded-[2.5rem] bg-linear-to-b from-primary/10 to-primary/5">
            <GlassCard className="p-10 space-y-10 bg-white/50 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-6 pt-10 border-t border-outline-variant/30">
                ...
                <div className="flex justify-between text-on-surface-variant font-bold text-sm uppercase tracking-widest">
                  <span>Platform Tax</span><span className="text-on-surface">$0.00</span>
                </div>
                <div className="flex justify-between items-end pt-6 border-t border-outline-variant/30">
                  ...
                  <span className="text-[10px] font-bold text-on-surface-variant mb-1">{item.price === 0 ? "" : "USD (VAT Incl.)"}</span>
                </div>
              </div>
