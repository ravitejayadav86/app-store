...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const res = await api.post(`/social/messages/${username}`, { content: newMessage });
      setMessages(prev => [...prev, { ...res.data, sender_id: currentUserId!, receiver_id: 0, is_read: false }]);
      setNewMessage("");
    } catch {
  ...
  return (
    <div className="flex flex-col h-screen pt-20">
      ...
      <div className="flex items-center gap-4 px-4 py-3 border-b border-outline-variant/30 bg-surface">
        ...
        <Link href={`/users/${username}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
          ...
          return (
            <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm ${
                ...
                <p>{msg.content}</p>
                <p className={`text-[10px] mt-1 ${isMe ? "text-on-primary/60" : "text-on-surface-variant"}`}>
