...
import { CheckCircle2, Clock, ShieldAlert, XCircle, User, Tag, Music, BookOpen, Trash2 } from "lucide-react";
...
function AdminContent() {
    ...
    const fetchPublishedApps = useCallback(async () => {
        try {
            const res = await api.get("/admin/published");
            setPublishedApps(res.data);
        } catch {
    ...
    useEffect(() => {
        ...
        fetchPendingApps();
        fetchPublishedApps();
    }, [status, router, fetchPendingApps, fetchPublishedApps]);
    ...
    const handleDelete = async (id: number, name: string) => {
        ...
        try {
            ...
            setPublishedApps((prev) => prev.filter((app) => app.id !== id));
        } catch (err: unknown) {
            const error = err as ApiError;
            toast.error(error.response?.data?.detail || "Failed to delete app.");
        } finally {
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 py-20">
            <div className="flex items-center gap-4 mb-12">
                ...
                <div>
                    ...
                    <p className="text-on-surface-variant font-medium mt-1">
                        Review, approve, and manage published applications.
                    </p>
                </div>
            </div>
            ...
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                <Clock size={22} className="text-yellow-500" /> Pending Review
            </h2>

            {apps.length === 0 ? (
                <GlassCard className="p-12 flex flex-col items-center text-center gap-4 border-dashed mb-12">
            ...
            {/* Published Apps */}
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
                <ShieldAlert size={22} className="text-red-500" /> Published Apps — Remove Unsafe Apps
            </h2>

            {publishedApps.length === 0 ? (
                <GlassCard className="p-12 flex flex-col items-center text-center gap-4 border-dashed">
                    <CheckCircle2 size={40} className="text-green-500/50" />
                    <p className="text-on-surface-variant">No published apps found.</p>
                </GlassCard>
            ) : (
                <div className="grid grid-cols-1 gap-6">
                    {publishedApps.map((app) => {
                        const category = app.category?.toLowerCase() || '';
                        return (
                            <GlassCard key={app.id} className="p-8 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center transition-all hover:border-red-500/20">
                                <div className="space-y-4 flex-1">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-2xl bg-surface-low flex items-center justify-center text-primary">
                                            {category === 'music' ? <Music size={24} /> :
                                ...
                                <button
                                    ...
                                    className="flex-none px-6 py-3 rounded-2xl border border-red-500/30 bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-all font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                                    <Trash2 size={18} /> {deleteLoading === app.id ? "Deleting..." : "Delete App"}
                                </button>
                            </GlassCard>
                        );
                    })}
